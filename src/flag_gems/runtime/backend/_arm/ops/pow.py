...
def _is_supported_tensor(t):
    return isinstance(t, torch.Tensor) and t.device.type == "cpu" and t.dtype in (
        torch.float16,
        torch.bfloat16,
        torch.float32,
        torch.float64,
...
def _launch_pow_kernel(multi_kernel, single_kernel, x, out_tensor, n_elements, block_size):
