...
def _rope_qk_bf16_kernel(
    ...
    row = tl.where(is_q, q_ptr + pid * head_dim,
                   k_ptr + (pid - n_heads_q) * head_dim)
    ...
    try:
        for hd in (128,):
            ...
            _rope_qk_bf16_kernel[(4,)](
                q, k, c, s, 2, hd, hd // 2,
                BLOCK_HALF=64, num_warps=1, num_stages=1,
            )
    except Exception:
...
def _rope_bf16_jit(q, k, cos_half, sin_half, n_heads_q, n_heads_kv, head_dim):
    ...
    _rope_qk_bf16_kernel[(total,)](
        q, k, cos_half, sin_half, n_heads_q, head_dim, half,
        BLOCK_HALF=64, num_warps=1, num_stages=1,
    )
...
def _patched_apply_rotary_pos_emb(q, k, cos, sin, unsqueeze_dim=1):
    ...
    if (q.dim() == 4 and k.dim() == 4
            and q.shape[0] == 1 and k.shape[0] == 1
            and q.shape[2] == 1 and k.shape[2] == 1
            and q.dtype == torch.bfloat16 and k.dtype == torch.bfloat16
            and cos.dim() in (2, 3, 4) and sin.dim() in (2, 3, 4)
        ...
        q_buf = q.clone()
        k_buf = k.clone()
        _rope_bf16_jit(q_buf, k_buf, cos_half, sin_half,
