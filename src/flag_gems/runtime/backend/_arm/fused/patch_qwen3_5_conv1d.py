...
def _causal_conv1d_update_kernel(
    hidden_ptr, state_ptr, weight_ptr, bias_ptr, out_ptr,
    B: tl.constexpr, C: tl.constexpr,
    kernel_size: tl.constexpr,
    silu: tl.constexpr, has_bias: tl.constexpr,
):
    _tle_causal_conv1d_update(
        hidden_ptr, state_ptr, weight_ptr, bias_ptr, out_ptr,
        B, C, kernel_size, silu, has_bias,
    )
...
def _make_patched_fn(torch_causal_fn):
    def fn(hidden_states, conv_state, weight, bias=None, activation=None):
        ...
        _causal_conv1d_update_kernel[(1,)](
            h, conv_state_c, w, b_t, out,
            B=B, C=C, kernel_size=4,
            silu=silu_flag, has_bias=has_bias,
        )
