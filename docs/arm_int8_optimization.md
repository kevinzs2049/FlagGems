...
| FlagGems fused | 6.82 tok/s (0.87x) | 5.38 tok/s (0.72x) |
...
| FlagGems fused | 6.16 tok/s (0.83x) | 4.70 tok/s (0.70x) |

Phase 2 fusion 相比 Phase 1（unfused）的提升：短 prompt +17~20%，长 prompt +12%。

---

## 剩余差距分析

### decode（短 prompt，M=1）差距：~0.83-0.87x

| 原因 | 估算损失 |
|------|---------|
| Triton i8mm GEMM 内核慢于 OneDNN/ACL | ~28 ms/tok |
...
### prefill（长 prompt，M=84）差距：~0.70-0.72x
