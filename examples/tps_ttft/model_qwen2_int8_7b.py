...
model_path = os.environ.get(
    "QWEN2_MODEL_PATH",
    "/home/kevin/.cache/modelscope/hub/models/qwen/qwen2-7b-instruct",
)
quant_model_path = os.environ.get("QWEN2_INT8_FULL_PATH")
disable_quant = os.environ.get("QWEN2_DISABLE_INT8", "0") == "1"
dtype_name = os.environ.get("QWEN2_DTYPE", "fp32").lower()
if dtype_name in ("bf16", "bfloat16"):
    model_dtype = torch.bfloat16
elif dtype_name in ("fp32", "float32"):
    model_dtype = torch.float32
else:
    raise ValueError(f"Unsupported QWEN2_DTYPE: {dtype_name}")

print("Loading tokenizer...")
tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)

print("Loading model...")
print(f"[Info] QWEN2_DISABLE_INT8={os.environ.get('QWEN2_DISABLE_INT8')} QWEN2_DTYPE={dtype_name} QWEN2_INT8_FULL_PATH={quant_model_path}")
if disable_quant:
    print(f"[Info] Loading non-int8 model (dtype={dtype_name})...")
    ...
    print(f"[Info] Loading int8 model: {quant_model_path}")
    model = torch.load(quant_model_path, map_location=device, weights_only=False)
    model.eval()
else:
    print("[Info] Loading FP32 model then quantize dynamic int8...")
...
def test_cpu_tps_and_first_token_latency(enable_gems):
    ...
    prompt_len = int(os.environ.get("QWEN2_PROMPT_TOKENS", "512"))
